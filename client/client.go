...
type SingleReportResponse struct {
	Report  SingleReport `json:"result"`
	Code    int          `json:"code"`
	Message string       `json:"message"`
}

type BulkReportResponse struct {
	Reports []SingleReport `json:"result"`
	Code    int            `json:"code"`
	Message string         `json:"message"`
}

const (
	path     = "/v1/api/reports/latest"
	bulkPath = "/v1/api/reports/bulk"
)
...
func GenerateHMAC(method string, path string, body []byte, clientId string, timestamp int64, userSecret string) string {
	...
	serverBodyHashString := fmt.Sprintf("%s %s %s %d",
		method,
		path,
		clientId,
		timestamp)
...
func FetchSingleReportSingleFeed(feedId string) (SingleReport, error) {
	...
	userSecret := os.Getenv("CLIENT_SECRET")
