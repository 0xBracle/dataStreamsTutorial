...
func handleSingleReport(feedId string) {
	...
	decodedReport, err := internal.DecodeFullReportAndReportData(fullReport)
	if err != nil {
		log.Fatalf("Failed to decode report: %v", err)
	}

	// 	// Print details of the decoded report
	var decodedReportsList []*internal.ReportWithContext
	decodedReportsList = append(decodedReportsList, decodedReport)
	printReportDetails(decodedReportsList)

	// 	// Print the full report in hex format as the payload
	printPayload(fullReport, feedId)
}

func handleMutipleReport(feedIds []string) {
